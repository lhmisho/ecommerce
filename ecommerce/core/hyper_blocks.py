...
from ecommerce.offer.models import ConditionalOffer
...
def offer_dict():
    offers = list(map(lambda x: {'id': x.id, 'name': str(x)}, ConditionalOffer.objects.all()))
    return json.dumps([{'id': 0, 'name': 'All Offers'}] + offers)
...
class ProductManager(StylableBlock):
    ...
    def get_context(self, value, parent_context=None):
        ...
        latest_arrival = value['settings'].get('latest', False)
        if categoryId == 0:
            if latest_arrival:
                context['products'] = Product.objects.filter(new_arrival=True)
            else:
                context['products'] = Product.objects.all()
        else:
...
@register("review_block")
class ReviewBlock(StylableBlock):
    JS_VARIABLES = {}

    JS_PLUGINS = 'core/djangohyper/review.js'


@register("offer")
class OfferBlock(StylableBlock):
    JS_VARIABLES = {
        'OFFERS': lazy(offer_dict)
    }

    JS_PLUGINS = 'core/djangohyper/offer_block.js'

    def get_context(self, value, parent_context=None):
        context = super().get_context(value, parent_context=parent_context)
        offerId = value['settings'].get('offerId', None)
        offers = None
        if offerId is not None:
            offers = ConditionalOffer.objects.filter(id=int(offerId)).first()
            # upsell_message = offers.get_upsell_message(
            #     self.request.basket)
        context['upsell_message'] = "Buy more than {} product and get {} ".format(int(offers.condition.value), offers.benefit.description)
